/**
 * Дополнительные страницы сайта («Цены», «Акции», «Как мы работаем») —
 * конструктор в админке, открываются по адресу /p/<slug>.
 *
 * Копия shared/pages.ts для мобильного приложения: Metro не умеет импортировать
 * файлы за пределами корня mobile/, поэтому модуль продублирован (как и контент
 * главной в content.ts, и типы заявок в api.ts). При правках в shared/pages.ts —
 * синхронизировать сюда.
 *
 * Здесь:
 *  - типы страницы и блоков,
 *  - PAGE_BLOCK_LABELS / PAGE_BLOCK_HINTS / newBlock / newPage — для редактора,
 *  - sanitizePages() — очистка данных перед отправкой на сервер (и для чтения).
 *
