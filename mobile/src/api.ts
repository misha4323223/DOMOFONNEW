...
/**
 * Подсказка о манёвре — из неё карта делает «через 300 м направо».
 * Считает не сервер, а телефон, по уже загруженной линии: так подсказка
 * обновляется на каждом обновлении геопозиции, без запросов в сеть.
 */
export interface GeoManeuver {
  /** «направо», «налево», «по кругу», «прибытие»… */
  text: string;
  /** Улица, на которую выезжаем; пустая строка — неизвестна. */
  street: string;
  lat: number;
  lon: number;
}
...
export interface GeoRideLeg {
  ...
  distance: number;
  duration: number;
  /** Повороты участка. Нет только в ответе старого сервера. */
