...
const SERVICE_KEYWORDS: { value: string; keys: RegExp }[] = [
  // Видеонаблюдение — раньше ремонта и установки: если в записи есть камера
  // или видеонаблюдение, заявка именно про них (иначе «камера не работает»
  // улетело бы в обычный ремонт домофона).
  { value: "cctv", keys: /(видеонаблюд|видео\s*наблюд|видеокамер|камер[аыуе]|регистратор|cctv|видеофон)/i },
